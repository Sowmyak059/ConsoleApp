    ...
    */
    ...
    class AssignmentW48
    {
        ...
        private void printTheEnteredData(String strSearchItem = "")
        {
            ...
            {
                productDetails.Sort((iSource, iDestination) => iSource.Price.CompareTo(iDestination.Price));
                Console.WriteLine("---------------------------------------------------------------------------------");
                Console.ForegroundColor = ConsoleColor.Green;
                ...
                foreach (ItemInfo info in productDetails)
                {
                    ...
                    if ((strSearchItem != "") && (info.ProductName.Equals(strSearchItem, StringComparison.OrdinalIgnoreCase)))
                    {
                ...
                if (strSearchItem == "")
                {
                    Console.WriteLine("\t\t\tToatal amount:".PadRight(28) + productDetails.Sum(item => item.Price));
                }
                Console.WriteLine("---------------------------------------------------------------------------------");
            }
        }
